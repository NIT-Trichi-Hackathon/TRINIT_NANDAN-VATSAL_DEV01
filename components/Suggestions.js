          ...
          {ngos.map((item) => {
            ...
            if (donationPreference == item.target) return;
              ...
              return (
                <Link
                  key={item._id}
                  className="space-y-3  w-1/4 px-4 border border-black"
                  passHref={true}
                  href={`/ngoDetails/${item.email}`}
                >
                  {
                    <div>
                      <div className="font-semibold text-2xl underline capitalize">
                        {item.name}
                      </div>
                      <div>
                        <div className="text-sm">
                          {" "}
                          Target area : {item.target}
                        </div>
